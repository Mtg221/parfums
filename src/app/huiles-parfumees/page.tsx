...
function HuilesParfumeesContent() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        console.error('Failed to load Huiles Parfumées data:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleAddToCart = (prod: Product) => {
    const price = prod.priceHuile || (prod.formats?.[0]?.price) || 5000;
    addItem({
      productId: prod.id,
      name: prod.name,
      brand: prod.brand,
      ...
      price: price,
      quantity: 1,
      formatName: 'Format Standard',
    });
    setAddedMessage(`"${prod.name}" ajouté au panier !`);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-7 space-y-4">
            ...
            <h1 className="text-4xl sm:text-5xl font-serif">
              <span className="font-bold text-stone-900 block uppercase">HUILES PARFUMÉES</span>
              <span className="italic font-serif text-[#C5928E] block -mt-1">Par Maisons de Parfum</span>
            </h1>

            <p className="text-xs sm:text-sm text-stone-600 font-light max-w-md leading-relaxed">
              Des senteurs en format pratique sans alcool, concentrées et idéales pour le quotidien. Sélectionnez une maison ci-dessous pour découvrir ses huiles.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-stone-200 pb-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-stone-500 font-light">
                Chaque maison propose des huiles parfumées inspirées des plus grandes fragrances.
              </p>
            </div>
          ...
          <div className="bg-[#FAF0ED] border border-[#E8D5D0] rounded-sm p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="text-xs text-stone-600 font-light mt-1">
                Découvrez toutes nos huiles parfumées hautement concentrées issues de la maison {selectedBrand}.
              </p>
            </div>
                    ...
                    >
                      <div className="space-y-3 relative">
                        ...
                        <div className="relative aspect-square bg-stone-50 rounded-sm overflow-hidden flex items-center justify-center p-2 border border-stone-100">
                          <Image
                            src={prod.imageUrl || "https://images.unsplash.com/photo-1615397349754-cfa2066a298e?auto=format&fit=crop&w=800&q=80"}
                            alt={prod.name}
                            fill
...
export default function HuilesParfumeesPage() {
  return (
    <Suspense fallback={
